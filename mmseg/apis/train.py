...
def train_active_learning_segmentor(model,
                                    dataset,
                                    cfg,
                                    distributed=False,
                                    validate=False,
    ...
    if cfg.get('runner') is None:
        cfg.runner = {'type': 'ActiveEpochBasedRunner', 'max_iters': cfg.total_iters}
        warnings.warn(
            'config is now expected to have a `runner` section, '
            'please set `runner` in your config.', UserWarning)
    ...
    runner = build_runner(
        ...
        default_args=dict(
            ...
            dataloader_cfg=train_loader_cfg, # this Active Learning Runner will receive config for training dataset so it
                                                # will be rebuilt every cycle in Runner
            model=model,
            batch_processor=None,
            optimizer=optimizer,
            work_dir=cfg.work_dir,
